...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self, adapter_name: str):
        ...
        elif config.peft_type == PeftType.EB_TUNING:
            prompt_encoder = EBencoder(config)
        else:
    ...
    def get_prompt_embedding_to_save(self, adapter_name: str) -> torch.Tensor:
        ...
        if self.peft_config[adapter_name].peft_type == PeftType.EB_TUNING:
            prompt_tokens = prompt_tokens[:, : self.peft_config[adapter_name].num_virtual_tokens]
    ...
    def get_prompt(self, batch_size: int, task_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING or peft_config.peft_type == PeftType.EB_TUNING:
...
class PeftModelForCausalLM(PeftModel):
    ...
    def prepare_inputs_for_generation(self, *args, task_ids: Optional[torch.Tensor] = None, **kwargs):
        ...
        if peft_config.is_prompt_learning:
            ...
            elif requires_prompt_injection and peft_config.peft_type == PeftType.EB_TUNING:
                new_past_key_values = self.get_prompt(batch_size=model_kwargs["input_ids"].shape[0])
                model_kwargs["past_key_values"] = new_past_key_values
